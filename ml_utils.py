...
:parameter
    ...
    :param max_cat: num - max number of uniques to consider a numerical variable as categorical
'''
def bivariate_plot(dtf, x='', y='', max_cat=20, figsize=(10,3),ax='',fig='',save='',scatter=True,title=''):
    '''
    bivariate plot provides bivariate analysis of two variables, 
    number vs number: satcked graph with density plot
    '''
    if (x=='')&(y==''):
        x=dtf.columns[0]
        y=dtf.columns[1]
    try:
        ...
        if (utils_recognize_type(dtf, x, max_cat) == "num") & (utils_recognize_type(dtf, y, max_cat) == "num"):
            ...
            if (fig=='') and (ax==''):
                fig, ax = plt.subplots(figsize=figsize)
            ax.set_title(title+' '+x+"   vs   "+y, fontsize=20)
            ...
            if ax=='':
                plt.show()
            ### joint plot
            #join plot contains density and scatter plot, default is dropna, includes a regression line
            if scatter:
        ...
        ## cat vs cat --> hist count + hist %
            ...
            if (fig=='') and (ax==''):
                ...
                ax.set_title(x+"   vs   "+y, fontsize=20)
                ### count
        ...
        ## num vs cat --> density + stacked + boxplot 
            ...
            if (fig=='') and (ax==''):
                ...
                for col in tmp.drop("tot", axis=1).columns:
                    tmp[col] = tmp[col] / tmp["tot"]
                tmp.drop("tot", axis=1)[sorted(dtf[cat].unique())].plot(kind='bar', stacked=True, ax=ax, legend=False, grid=True)
        
    except Exception as e:
        print("--- got error ---")
        print(utils_recognize_type(dtf, x, max_cat))
        print(utils_recognize_type(dtf, y, max_cat))
        print(e)
